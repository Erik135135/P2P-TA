...
df_spot_price = spot_market_API_enstoe(contry_code=Country_code, startdate=time_start, enddate=time_end)
...
if data_no_spot == 'False' and data_match == 'Yes':
    if Time_start_equal == 'Yes' and Time_end_equal == 'Yes':
        if one_day_forecast == 'Defined time':
            ...
            if time_end_date.date() > dt_spot_high.date():
                time_end_date = dt_spot_high.date() + datetime.timedelta(days=1)
                time_end = time_end_date.strftime("%Y%m%d")
                text_all.append('There is no spot_price after ' + str(dt_spot_high.date()) + '23:55.')
...
if data_no_spot == 'False' and data_match == 'Yes':
    ...
    bm.add_bids_from_API(frequency_ratio=frequency_ratio, freq_market=frequency_market, demand=demand, production=production)
    #
    spot_tot, a, b = bm.add_spot_prices(freq=frequency_market)
